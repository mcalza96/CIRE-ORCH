...
async def test_generate_best_effort_recency_ordering(adapter, mock_service):
    ...
    chunks = [
        EvidenceItem(source="C1", content="old", metadata={"row": {"metadata": {"updated_at": "2020-01-01T00:00:00Z"}}}),
        EvidenceItem(source="C2", content="new", metadata={"row": {"metadata": {"updated_at": "2024-01-01T00:00:00Z"}}}),
    ]
...
async def test_generate_adds_traceability_note_when_bridge_missing(adapter, mock_service):
    plan = create_retrieval_plan(mode="comparativa", standards=("ISO 9001", "ISO 14001"), require_literal=False)
    
    # Two chunks, each mentioning only one standard
    chunks = [
        EvidenceItem(source="C1", content="About ISO 9001 requirements", metadata={"row": {"content": "About ISO 9001", "metadata": {"source_standard": "ISO 9001"}}}),
        EvidenceItem(source="C2", content="About ISO 14001 requirements", metadata={"row": {"content": "About ISO 14001", "metadata": {"source_standard": "ISO 14001"}}}),
    ]
    
    # Answer mentions both
    mock_service.generate_answer = AsyncMock(return_value="The relation between ISO 9001 and ISO 14001 is...")
    
    result = await adapter.generate(query="comparing standards", scope_label="s", plan=plan, chunks=chunks, summaries=[])
...
async def test_generate_no_note_if_bridge_chunk_exists(adapter, mock_service):
    ...
    chunks = [
        EvidenceItem(source="C1", content="ISO 9001 and ISO 14001 together", metadata={"row": {"content": "ISO 9001 and ISO 14001", "metadata": {}}}),
    ]
    
    mock_service.generate_answer = AsyncMock(return_value="Answer mentioning ISO 9001 and ISO 14001")
    
    result = await adapter.generate(query="q", scope_label="s", plan=plan, chunks=chunks, summaries=[])
...
async def test_generate_no_note_if_already_disclosed(adapter, mock_service):
    ...
    chunks = [
        EvidenceItem(source="C1", content="ISO 9001", metadata={"row": {"content": "ISO 9001", "metadata": {}}}),
        EvidenceItem(source="C2", content="ISO 14001", metadata={"row": {"content": "ISO 14001", "metadata": {}}}),
    ]
    
    # Answer already contains disclosure keyword
    mock_service.generate_answer = AsyncMock(return_value="Basado en una inferencia de ISO 9001 y ISO 14001...")
