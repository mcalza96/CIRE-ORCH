...
def test_classify_intent_literal_lista():
    intent = classify_intent("Lista las entradas exclusivas de la cláusula 9.1", profile=ISO_PROFILE)
    # V2 prefiere literal_lista para queries tipo "Lista..." con referencia
    assert intent.mode in ("literal_lista", "ambigua_scope")


def test_classify_intent_comparativa():
    intent = classify_intent("Compara ISO 27001 vs ISO 9001 para proveedores externos", profile=ISO_PROFILE)
    # Si detecta multiples scopes e intencion comparativa
    assert intent.mode in ("comparativa", "literal_lista")
...
def test_suggest_scope_candidates_uses_domain_hints():
    options = suggest_scope_candidates("requisitos legales ambientales de la clausula 9.1.2", profile=ISO_PROFILE)
    assert "ISO 14001" in options


def test_detect_scope_candidates_supports_bare_iso_numbers():
    options = detect_scope_candidates("impacta 9001, 14001 y 45001 por ciberataque", profile=ISO_PROFILE)
