...
def _finder_select_files() -> list[str]:
    if sys.platform != "darwin":
        print("⚠️ Selección por Finder solo está disponible en macOS.")
        return []
    command = [
        "osascript",
        "-e",
        'set selectedFiles to choose file with prompt "Selecciona uno o más archivos" with multiple selections allowed',
        "-e",
        "set outputLines to {}",
        "-e",
    ...
    return [candidate for candidate in selected if os.path.isfile(candidate)]


def _finder_select_folder() -> list[str]:
    if sys.platform != "darwin":
        print("⚠️ Selección por Finder solo está disponible en macOS.")
        return []
    command = [
        "osascript",
        "-e",
        'set selectedFolder to choose folder with prompt "Selecciona una carpeta"',
        "-e",
    ...
    if result.returncode != 0:
        return []
    folder = str(result.stdout or "").strip()
    if not folder:
        return []
    return expand_files_from_path(folder)


def collect_files_interactively() -> list[str]:
    if sys.platform == "darwin":
        print("🗂️ Abriendo Finder para seleccionar archivos...")
        picked = _finder_select_files()
        if picked:
            print(f"✅ Finder seleccionó {len(picked)} archivo(s).")
            return picked
        folder_files = _finder_select_folder()
        if folder_files:
            print(f"✅ Finder seleccionó carpeta con {len(folder_files)} archivo(s).")
            return folder_files
        print("ℹ️ No se seleccionaron archivos en Finder. Continuamos con ingreso manual.")

    while True:
        path_raw = prompt(
            "📝 Ruta al archivo/carpeta (glob ok) o escribe 'finder' para seleccionar: "
        )
        finder_key = str(path_raw or "").strip().lower()
        if finder_key in {"finder", "/finder", ":finder"}:
            picked = _finder_select_files()
            if picked:
                print(f"✅ Finder seleccionó {len(picked)} archivo(s).")
                return picked
            print("⚠️ Finder no devolvió archivos. Puedes reintentar o seleccionar carpeta.")
            folder_files = _finder_select_folder()
            if folder_files:
                print(f"✅ Finder seleccionó carpeta con {len(folder_files)} archivo(s).")
                return folder_files
            continue
        if not path_raw:
            print("⚠️ Debes indicar una ruta/glob o usar 'finder'.")
            continue
...
async def pick_documents_for_replay(
    *,
    ...
    if args.non_interactive:
        ...
        replay_doc_ids = list(dict.fromkeys(replay_doc_ids))
        if not replay_doc_ids:
            raise RuntimeError(
                "No hay documentos válidos para replay en la colección seleccionada."
            )
        print(
            f"🚀 Replay en lote: {len(replay_doc_ids)} documento(s) de la colección seleccionada."
        )
        return replay_doc_ids

    print("🧭 Modo replay para la colección:")
    print("  1) Todos (solo grafos) (Recomendado)")
    print("  2) Elegir documento único")

    mode = ""
    while True:
        raw_mode = prompt("📝 Elige opción [1-2]: ")
        if raw_mode in {"1", "2"}:
            mode = raw_mode
            break
        print("Opción inválida.")

    replay_doc_ids: list[str] = []
    for doc in selected_docs:
        doc_id = str(doc.get("id") or "").strip()
        if doc_id:
            replay_doc_ids.append(doc_id)

    replay_doc_ids = list(dict.fromkeys(replay_doc_ids))
    if not replay_doc_ids:
        raise RuntimeError("No hay documentos válidos para replay en la colección seleccionada.")
    ...
    print(f"🚀 Replay en lote: {len(replay_doc_ids)} documento(s) de la colección seleccionada.")
    return replay_doc_ids
