...
def extract_requested_scopes(query: str, profile: AgentProfile | None = None) -> tuple[str, ...]:
    ...
    def _digits_signature(value: str) -> tuple[str, ...]:
        return tuple(sorted(set(re.findall(r"\d{2,6}", str(value or "")))))

    def _is_digits_only_scope(value: str) -> bool:
        compact = re.sub(r"\s+", "", str(value or ""))
        return bool(re.fullmatch(r"\d{3,6}", compact))

    def _add_scope(candidate: str) -> None:
        value = _canonical_scope_value(candidate)
        if not value or value in found:
            return

        value_digits = _digits_signature(value)
        if value_digits:
            for idx, existing in enumerate(list(ordered)):
                existing_digits = _digits_signature(existing)
                if existing_digits != value_digits:
                    continue
                existing_is_numeric = _is_digits_only_scope(existing)
                value_is_numeric = _is_digits_only_scope(value)

                # Prefer richer labels when both refer to the same numeric scope
                # (e.g., keep "ISO 9001" over "9001").
                if existing_is_numeric and not value_is_numeric:
                    found.discard(existing)
                    ordered[idx] = value
                    found.add(value)
                    return
                if not existing_is_numeric and value_is_numeric:
                    return
